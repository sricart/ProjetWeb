...
{
	const communeUrl = 'https://geo.api.gouv.fr/communes?codePostal=';
	...
	$(zipcode).on('blur', function(){
		let code = $(this).val();
		//console.log(code);
		let urlC = communeUrl+code+format;
		//console.log(url);

		fetch(urlC, {method: 'get'}).then(response => response.json()).then(results => 
			{
				//console.log(results);

...
if($_POST){
    ...
    && isset($_POST['Pilote'])){
        ...
        $pil = strip_tags($_POST['Pilote']);

        $sql = 'UPDATE `etudiant` INNER JOIN `authentifiant` ON etudiant.Id_Auth = authentifiant.Id_Auth SET `N_Etudiant`=:nom, `P_Etudiant`=:prenom, `Cv`=:cv, `Lettre_Motivation`=:lm, `Photo`=:photo, `Id_Promotion`=:promo, `Id_Pilote`=:pilote,`Login`=:log, `Mdp`=:mdp, `Admin`=:adm, `Pilote`=:pil WHERE `Id_Etudiant`= :id;';
        ...
        $query->bindValue(':adm', $adm, PDO::PARAM_INT);
        $query->bindValue(':pil', $pil, PDO::PARAM_INT);
        ...
        <div class="row">
            <section class="col-12">
                ...
                <form method="post">
                    ...
                    <div class="form-group">
                        <label for="Admin">Statut Admin</label>
                        <input type="text" id="Admin" name="Admin" class="form-control" value="<?= $etudiant['Admin']?>">
                    </div>
                    <div class="form-group">
                        <label for="Pilote">Statut Pilote</label>
                        <input type="text" id="Pilote" name="Pilote" class="form-control" value="<?= $etudiant['Pilote']?>">
                    </div>
